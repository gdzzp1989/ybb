...
public class SocketServer {
	 ...
	 * channel的事件。 于是我们拿到新的socket channel后，要做的第一个事情就是到selector“选择器/代理器”中注册这个 socket
	 * channel感兴趣的事件
	 * 
	 * @param socketChannel
	 *            新的socket channel
	...
	private static void readSocketChannel(SelectionKey readyKey) throws Exception {
		...
		if (URLDecoder.decode(message.toString(), "UTF-8").indexOf("over") != -1) {
			...
			SocketServer.LOGGER.info("端口:" + resoucePort + "客户端发来的完整信息======completeMessage : "
			...
			SocketServer.LOGGER.info("端口:" + resoucePort + "客户端信息还未接受完，继续接受======message : "
