...
public class ZKDemo {
	...
	private static void authCreate() throws Exception {
		...
		String str = authZK.create(path + "/auth", "test".getBytes(), Ids.CREATOR_ALL_ACL, CreateMode.EPHEMERAL);
	...
	private static void createZKWithSessionIdAndSessionPasswd() throws Exception {
		...
		/**
		 * 创建一个ZooKeeper对象，192.168.133.149:2180是zk服务器的主机名和端口号 50000 sessionTimeout
		 * session超时时间 Watcher 默认的WatchedEvent事件处理器
		 */
	...
	private static ZooKeeper sync() throws Exception {
		...
		/**
		 * 创建一个ZooKeeper对象，localhost:2181是zk服务器的主机名和端口号 50000 sessionTimeout session超时时间
		 * Watcher 默认的WatchedEvent事件处理器
		 */
		ZooKeeper zk = new ZooKeeper("192.168.133.149:2180", 50000, new Watcher() {

			public void process(WatchedEvent event) {
				System.out.println("默认监听器，KeeperStat:" + event.getState() + ", EventType:" + event.getType() + ", path:"
						+ event.getPath());
				/**
				 * 如果zk客户端和服务器已经建立连接，闭锁减一
				 */
		...
		zk.getData(path, true, new DataCallback() {
			...
			public void processResult(int rc, String path, Object ctx, byte[] data, Stat stat) {
				System.out.println("rc=" + rc + ",path=" + path + " data=" + data + " Stat=" + stat + ", data="
		...
		/**
		 * 如果该节点不存在则创建持久化的节点 ZK的节点有三位，持久化节点（PERSISTENT）,临时节点（EPHEMERAL），顺序节点（SEQUENTIAL）
		 * 具体的组合有 /**
		 * 
		 * PERSISTENT, //持久化序列节点 PERSISTENT_SEQUENTIAL,
		 * 
		 * EPHEMERAL , //临时序列节点 EPHEMERAL_SEQUENTIAL
		 * 
		 */
		...
		try {
			zk.delete(path + "/seq ", 1, new VoidCallback() {
				...
				public void processResult(int rc, String path, Object ctx) {
					System.out.println("删除" + ZKDemo.path + "/seq 的结果是：rc=" + rc + " path=" + path + ",context=" + ctx);
				}
			}, null);
		} catch (Exception e) {
		...
		// 创建一个持久化序列节点
		String seqPath = zk.create(path + "/seq ", null, Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
		// 读取子节点列表，并注册一个监听器
		...
		// 异步创建临时序列节点
		zk.create(path + "/test", "123".getBytes(), Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL_SEQUENTIAL,
				new StringCallback() {

					public void processResult(int rc, String path, Object ctx, String name) {
						System.out.println("创建临时节点的结果是：result code=" + rc + ", path=" + path + " context=" + ctx
								+ ", name=" + name);
					}
				}, "Test Context");
		...
		for (String str : znodes) {
			...
			System.out.println("获取节点：" + str + " 的数据是：" + (data == null ? "null" : new String(data)));
		}
		return zk;
	}
}
